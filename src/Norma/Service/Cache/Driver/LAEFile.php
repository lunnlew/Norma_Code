...
class LAEFile extends Base {
	...
	public function flush() {
		...
		$dir = @opendir($path);
		if (!$dir) {
			throw new \Norma\Exception\RuntimeException("Can't read PATH:" . $path, 94);
		}
		...
		while ($file = readdir($dir)) {
			if ($file != "." && $file != ".." && is_dir($path . "/" . $file)) {
				...
				if (!$subdir) {
					throw new \Norma\Exception\RuntimeException("Can't read path:" . $path . "/" . $file, 93);
				}
	...
	*/
	public function readfile($file) {
		if (function_exists("file_get_contents")) {
			...
			if (!$file_handle) {
				throw new \Norma\Exception\RuntimeException("Can't Read File", 96);
		...
		if ($skip == false) {
			if (!file_exists($path)) {
				if (!@mkdir($path, 0777, true)) {
					throw new \Norma\Exception\RuntimeException("PLEASE CHMOD " . $this->getPath() . " - 0777 OR ANY WRITABLE PERMISSION!", 92);
				}
