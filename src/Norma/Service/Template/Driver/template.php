...
namespace Norma\Service\Template\Driver;
...
class Template {
	...
	public function __construct(array $config = []) {
		...
		$class = false !== strpos($type, '\\') ? $type : '\\Norma\\Service\\Template\\Driver\\' . ucwords($type);
	...
	private function parseTemplateFile($template) {
		...
		if (is_file($template)) {
			...
			throw new \Norma\Exception\TemplateNotFoundException('template not exists:' . $template);
		}
	}
