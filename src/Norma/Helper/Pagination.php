...
class Pagination
{
    ...
    public function setSourceCall($callback, $param = array(), $attr = 'data', $cover = 1, $delimiter = '::')
    {
        ...
        } elseif (is_array($callback)) { //支持array(func),array(instance,method)
            $call = $callback;
        }
        switch (count($call)) {
            case 1:
        ...
        //注意:强制转化为array
        if ($cover==1) {
            $this->$attr=array_merge((array) $this->$attr, (array) $data);
        } else {
            $this->$attr=$data;
        }
    }
     ...
     */
    public function makePagination($baseurl = '', $callback = '')
    {
    ...
    //获取样式模版
    public function getTemplate($name = 'style.php')
    {
    ...
    //获取容器模版
    public function getContainer($name = 'container.php')
    {
        $filename = $this->styleDir.$this->styleName.'/'.$name;
        if (file_exists($filename)) {
            $style = include_once($filename);
    ...
    //返回样式url
    public function geStyleUrl($styleUrl, $name = 'style.css')
    {
