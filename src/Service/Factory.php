...
class Factory
{
    ...
    protected static $tpl = '\Service\%s\Drive\%s';
    /**
     * 服务实例化函数
     *
     ...
     * @return object 驱动实例
     */
    public static function getFactory($name = '', $options = array(), $default = 'Service', $prex = 'Norma')
    {
        ...
        $new_name = self::getServiceName($name);
     ...
     * @return object 驱动实例
     */
    public static function getFactoryByOriginal($name = '', $options = array(), $default = 'Service', $prex = 'Norma')
    {
        $called_class = get_called_class();
        $class_parts = explode('\\', $called_class);
        $server_name = array_pop($class_parts);
    ...
    {
        $class = static::getRealServiceName($class, $prex);
