...
class BaseV2 {
		...
		if (isset($this->api_params[$this->api_name])) {
			if (isset($this->api_params[$this->api_name]['url'])) {
				...
				if (in_array('GET', $methods, true) && isset($this->api_params[$this->api_name]['jump'])) {
					$this->_jumpUrl($this->api_name, $this->_parseRequestParams(), array_shift($methods));
				} else {
					//获取数据
					return $this->_fetchUrl($this->api_name, $this->_parseRequestParams(), array_shift($methods));
				}
			} else {
				//获取数据
				return $this->_fetchData($this->api_name, $this->_parseParams());
			}
		} else {
			return null;
		}
	 ...
	 * @return array 结果
	 */
	protected function _parseRequestParams() {
	...
	protected function _parseParams() {
		foreach ($this->access_params as $name => $value) {
			if (is_string($value)) {
				$access_params[] = $name . '|@' . $value;
			} else {
				$access_params[] = $name . '|@' . array_pop($value); //对41行的处理,具有多个相同key时
			}
		}
		$this->access_params = array_unique($access_params);
		foreach ($this->access_params as $str) {
			$this->runtime_params = array_merge($this->runtime_params, $this->_parseStr($str));
		}
		$this->access_params = array();
	...
	*/
	...
	protected function _fetchUrl($name, $params = array(), $method = 'GET') {
		...
		//     $cookies = $params['header']['cookie'];

		// }unset($params['header']['cookie']);
	 ...
	 * @return string 结果
	 */
	protected function _fetchData($name, $params = array()) {
		array_walk($params, 'self::array_walk_call');
		print_r($params);
	}
	protected function array_walk_call(&$value, $name) {
		$params_call = $this->api_params[$this->api_name]['params_call'][$name];
		$value = call_user_func(array($this, $params_call), $value);
	}
