...
use Norma\Support\Traits\Macroable;
...
class Str
{
     ...
     * 转换UTF-8到ASCII.
     *
     * @param  string  $value 要转换的字串
     * @return string ascii字串结果
     */
     ...
     * 判断字符串中是否含有字串
     *
     * @param  string  $haystack 被搜索字符串
     * @param  string|array  $needles 需搜索字符串
     * @return bool
     */
     ...
     * @param  int     $limit 显示长度
     * @param  string  $end 结尾符号
     * @return string
     */
    ...
    /**
     * 分析Class@method形式字符串到Class和Method.
     *
    ...
    public static function random($length = 16)
    {
        ...
        while (($len = strlen($string)) < $length) {
            ...
            $bytes = static::random_bytes($size);
	 ...
	 * 
     * @param  int  $length 随机数长度
     * @return string bytes 二进制串
     *
     * @throws \RuntimeException
     */
    public static function random_bytes($length = 16)
    {
     ...
     * 生成字母数字的随机串.
     *
     * 对于加密等安全为是不合适的,例如加盐salts,keys等等
     *
     * @param  int  $length
     * @return string
     */
     ...
     * 恒定时间字符串比较.
	 * 
	 * 本函数可以用在需要防止时序攻击的字符串比较场景
     * 
     *
     * @param  string  $knownString  已知字符串
     * @param  string  $userInput 用户提供的字符串
     * @return bool 相等时返回true,否则false
     */
    public static function equals($knownString, $userInput)
    {
    	//处理字符串不相等时的时长问题
    	if($ret = strlen($knownString)^strlen($userInput)){
    		$ret |= array_sum(unpack("C*", $knownString^$knownString));
    	}else{
    		$ret = !hash_equals($knownString, $userInput);
    	}
		return !$ret;
    }
