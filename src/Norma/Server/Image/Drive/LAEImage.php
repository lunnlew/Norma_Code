...
final class LAEImage extends Base
{
    ...
    public $width = '';
    public $height = '';
    public $mode = \Norma\Server\Image\Mode::WIDTH;
    ...
    public function thumb($mode = \Norma\Server\Image\Mode::WIDTH)
    {
        $params=array();
        //宽度上下限
        if (($mode&\Norma\Server\Image\Mode::MAXWIDTH)
            &&($mode&\Norma\Server\Image\Mode::MINWIDTH)
            ) {
            $this->width = array_pad($this->width, 2, 0);
            sort($this->width);
            list($params['minwidth'],$params['maxwidth']) = $this->width;
        } elseif ($mode&\Norma\Server\Image\Mode::MAXWIDTH) {
            //宽度上限
            $params['maxwidth'] = $this->width[0];
        } elseif ($mode&\Norma\Server\Image\Mode::MINWIDTH) {
            //宽度下限
            $params['minwidth'] = $this->width[0];
        }
        //高度上下限
        if (($mode&\Norma\Server\Image\Mode::MAXHEIGHT)
            &&($mode&\Norma\Server\Image\Mode::MINHEIGHT)
            ) {
            $this->height = array_pad($this->height, 2, 0);
            sort($this->height);
            list($params['minheight'],$params['maxheight']) = $this->height;
        } elseif ($mode&\Norma\Server\Image\Mode::MAXHEIGHT) {
            //高度上限
            $params['maxheight'] = $this->height[0];
        } elseif ($mode&\Norma\Server\Image\Mode::MINHEIGHT) {
            //高度下限
            $params['minheight'] = $this->height[0];
        }
        //指定宽度
        if ($mode&\Norma\Server\Image\Mode::WIDTH) {
            $params['width'] = $this->width[0];
        }
        //指定高度
        if ($mode&\Norma\Server\Image\Mode::HEIGHT) {
